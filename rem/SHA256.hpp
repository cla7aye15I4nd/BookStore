...
namespace sjtu{
    ...
    inline word_t SIGMA0(word_t x) { return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22); }
    inline word_t SIGMA1(word_t x) { return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25); }
    ...
    struct sha256_engine{
        ...
        std::string sha256sum(const std::string& message) {
            ...
            for (size_t i = 0; i < message.length(); ++i) tmp[i] = message[i];
            ...
            for (size_t i = 0, j = 0; i < length; i += 4, ++j)
                plain[j] = (tmp[i] << 24) | (tmp[i + 1] << 16) | (tmp[i + 2] << 8) | tmp[i + 3];
